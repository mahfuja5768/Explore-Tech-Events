        ...
        {user && (
          <>
            <img
              ...
              alt="user photo"
            />
            <h3 className="mx-5 text-[#ffed48]">{user.displayName}</h3>
          </>
        )}
        {user ? (
          <Link to="/login" onClick={handleLogout} className="btn md:btn-sm normal-case">
            Logout
          </Link>
        ) : (
          <Link to="/login" className="btn md:btn-sm normal-case">
            Login
          </Link>
        )}
      </div>
